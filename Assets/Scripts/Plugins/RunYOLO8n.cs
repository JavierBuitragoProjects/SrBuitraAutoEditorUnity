...
public class RunYOLO8n : MonoBehaviour
{
    ...
    private UnityEngine.Transform displayLocation;
    ...
    void LoadModel()
    {
        ...
        var model2 = new FunctionalGraph {
        ...
        //FunctionalTensor modelOutput = model1.Forward(input)[0];
        ...
        //FunctionalTensor scores = FF.ReduceMax(allScores, 0) - scoreThreshold;        //shape=(8400)
        //FunctionalTensor classIDs = FF.ArgMax(allScores, 0);                          //shape=(8400) 
        ////FunctionalTensor boxCorners = FF.MatMul(boxCoords, model1.from centersToCorners);
        //FunctionalTensor indices = FF.NMS(boxCorners, scores, iouThreshold);           //shape=(N)
        //FunctionalTensor indices2 = indices.Unsqueeze(-1).BroadcastTo(new int[] { 4 });//shape=(N,4)
        //FunctionalTensor coords = FF.Gather(boxCoords, 0, indices2);                  //shape=(N,4)
        //FunctionalTensor labelIDs = FF.Gather(classIDs, 0, indices);                  //shape=(N)
        
        //Create engine to run model
        engine = new Worker(model2.Compile(), backend);
    }
